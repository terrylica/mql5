...
{
   /*
    * CONFIRMATION MARKERS OVERVIEW:
    * -----------------------------
    * This function adds visual markers at points where a zigzag reversal is confirmed.
    * - A peak is confirmed when we later find a bottom
    * - A bottom is confirmed when we later find a peak
    * 
    * These markers are useful to visualize when a reversal was actually confirmed,
    * which is always after the reversal itself occurred.
    * 
    * When porting to other languages, ensure this is treated as a purely visual feature
    * that doesn't affect the core zigzag algorithm.
    */
   ...
   // STEP 1: First pass - scan all bars to identify zigzag points
   for(int currentBarIndex=0; currentBarIndex<rates_total-1; currentBarIndex++)
     {
      // Check if current bar contains a peak
      bool isCurrentBarPeak = (ZigzagPeakBuffer[currentBarIndex] != 0);
      
      // Check if current bar contains a bottom
      bool isCurrentBarBottom = (ZigzagBottomBuffer[currentBarIndex] != 0);
      
      // STEP 2: Process peak confirmations - look for a bottom after this peak
      if(isCurrentBarPeak)
        {
         // Look ahead for a bottom that confirms this peak
         for(int futureBarIndex=currentBarIndex+1; futureBarIndex<rates_total; futureBarIndex++)
           {
            bool hasFoundConfirmingBottom = (ZigzagBottomBuffer[futureBarIndex] != 0);
            
            if(hasFoundConfirmingBottom)
              {
               // Mark confirmation of peak with an up arrow at the bottom point
               // We place it at the high price of the confirmation bar for visibility
               ConfirmPeakBuffer[futureBarIndex] = high[futureBarIndex];
               break; // Stop after finding the first confirmation
              }
           }
        }
      
      // STEP 3: Process bottom confirmations - look for a peak after this bottom
      else if(isCurrentBarBottom)
        {
         // Look ahead for a peak that confirms this bottom
         for(int futureBarIndex=currentBarIndex+1; futureBarIndex<rates_total; futureBarIndex++)
           {
            bool hasFoundConfirmingPeak = (ZigzagPeakBuffer[futureBarIndex] != 0);
            
            if(hasFoundConfirmingPeak)
              {
               // Mark confirmation of bottom with a down arrow at the peak point
               // We place it at the low price of the confirmation bar for visibility
               ConfirmBottomBuffer[futureBarIndex] = low[futureBarIndex];
               break; // Stop after finding the first confirmation
              }
           }
        }
                         ...
                         int &last_low_pos)
{
   ...
   if(prev_calculated > 0)
     {
      // We need to find the point where we should resume calculation
